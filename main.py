...
def sign_in():
	try:
		...
		if _code and _id and _lat and _long:
			...
			cursor.execute("select event_type_id, event_start, event_end, event_lat, event_long from events where event_id=" + eid)
			rows = cursor.fetchall()
			if rows:
				row = rows[0]
				...
				if row["event_start"] <= curr and curr <= row["event_end"]:
					...
					if row["event_lat"] == _lat and row["event_long"] == _long:
						cursor.execute("update members set member_points=member_points + (select type_points from types where type_id=" + str(row["event_type_id"]) + ") where member_id=" + _id)
